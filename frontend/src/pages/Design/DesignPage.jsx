...
<div className="relative w-full max-w-screen-xl mx-auto bg-white">
    ...
    <div className="relative z-10 flex flex-col items-center text-center mt-12">
        <h2 className="text-4xl font-semibold font-montserrat text-black mb-4">
        Wedding Invitation Presets
        </h2>
        <p className="text-lg text-black mx-6 mb-6">
    ...
    <div className="absolute top-[400px] w-full flex flex-col items-center text-center">
        <h3 className="text-3xl font-bold font-montserrat text-black mb-4">
        See What’s Popular
        </h3>
    ...
    <section>
    <div
        className="relative w-full py-12 bg-gradient-to-br from-[#FFF2F2] to-[#FFFFFF] flex justify-center items-center"
        style={{ overflow: "hidden" }}
    >
        ...
        {imagesEksklusif.map((image, index) => (
            <div
            key={index}
            className="border border-yellow-600 rounded-md flex flex-col justify-between items-center p-4 relative group"
            >
            <div className="w-full flex justify-center">
                <img
                src={image.src}
                alt={image.alt}
                className="w-4/5 transition-opacity duration-300 group-hover:opacity-50"
                />
            </div>
            ...
            <div
                className="absolute inset-0 flex justify-center items-center bg-pink-100 bg-opacity-70 text-black text-xl font-semibold opacity-0 transition-opacity duration-300 group-hover:opacity-80 cursor-pointer"
                onClick={() => handleNavigation(`/VIPDesign`)}
            >
                VIP
            </div>
            </div>
...
>
    ...
    {imagesPremium.map((image, index) => (
        <div
        key={index}
        className="border border-purple-600 rounded-md flex flex-col justify-between items-center p-4 relative group"
        >
        <div className="w-full flex justify-center">
            <img
            src={image.src}
    ...
    >
        ...
        {imagesBasic.map((image, index) => (
            <div
            key={index}
            className="border border-grey-600 rounded-md flex flex-col justify-between items-center p-4 relative group"
            >
            <div className="w-full flex justify-center">
                <img
                src={image.src}
